...
**Current step:** 5, dashboard. **Verified, awaiting commit.** Next: step 6
**Last verified:** Step 5. Checks 1–8 were run and passed. Check 9 (flags 12 and 13, and the page's error branch) was skipped. See the checkpoint
...
| 6 · All + My Change Controls | ⬜ | One list, two routes (decision 29) |
...
| 4 | ~~`docs/openapi.yaml` here has diverged from the backend's copy~~ **Closed 2026-09-11.** Lain diffed the two copies first. Every difference was one of the eleven fixes, and nothing existed only on the backend side. Lain then copied this file over the backend's, committed it there, rebuilt `server.exe` and restarted the Windows service. Verified at `localhost:1304/docs`: `/refresh`'s 401 cause table, the `active` parameter, and the three new schemas. ⚠️ **Defects 12–15 (steps 3 and 4) put the two copies apart again**, in the auth and users sections | Closed. 12–15 are **not worth a rebuild on their own**, so they go into the backend with its next rebuild, the same way |
...
| 12 | **The restore's failure branch and its Retry are unproven.** The API-down check was skipped at step 4. Every other path in `restore()` was exercised | To prove: stop the Windows service and reload any `(app)` page. Expect the error and Retry, with `ea-qms.refresh_token` still stored. Start the service and press Retry; expect the dashboard with no re-login. **Since step 5, the same outage also proves the dashboard's own error branch:** while signed in, stop the service, then go from Settings back to Dashboard. Expect the error in `.esig-error` and the header still rendered. Skipped again at step 5 |
...
| 14 | **These links 404 until their steps land:** `/change-controls` and its `?state=` stat-card links (step 6), `/approvals` (step 11), `/my-change-controls` (step 6), the Admin's `/settings/users` (step 15), and, since step 5, the dashboard's eye links to `/change-controls/{ccId}` (step 7a) | Accepted. Each stops 404ing when its step lands |
...
| 20 | **The prototypes' Ownership filter offers choices the API can't tell apart.** The schema has no `created_by` column, and create inserts only `change_owner_id` and `last_updated_by_id`. So "Created by me", "Owned by me" and My Change Controls' "Created by me or Owned by me" are all `owner=me`. Only "Awaiting my approval" (`assigned=me`) is different | Decide at step 6. B12's precedent is to drop list controls the API doesn't support (decision 11) |
...
| 22 | **Three helpers live inline in the dashboard page**: `BADGE` (state → class), `formatDateTime` and `stateHref`. Step 6's list needs all three | Decide at step 6. They are functions, not markup, so the three-copies rule is not the test. Lain decides whether they move to `lib/` |
| 23 | **The Create button is a placeholder.** It is disabled, with `title="Available at step 7a+"` (decision 31) | At 7a+: add the `onclick`, and remove both `disabled` and `title` |
...
| **`openapi.yaml`** | Hand-written from the handler code — a transcription, so **not infallible**. If a response disagrees, check the Go handler and fix the spec. **Fifteen defects found so far**: six at step 2, three more on a second reading of the same file, two in the A1 audit, two in step 3's sweep of every 401, and two in step 4's A10 audit — so treat this as a live warning, not a formality. **The count keeps rising because the checking keeps going**, not because the spec is unusually bad |
