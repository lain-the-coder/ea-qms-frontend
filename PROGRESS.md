...
**Current step:** 7d+, Navigation guard. **Verified, awaiting commit.** Next: step 8 (`Initiated` role views)
**Last verified:** Step 7d+. A forced sign-out while dirty reached `/login` with no prompt. A same-document Back under throttling showed "Loading…" with no Save Draft and no hint, which is the first exercise of the fix for a defect shipped at 7c. See the checkpoint
