...
**Current step:** 10, T3 cancel. **Verified, awaiting commit.** Next: step 11 (approver flow, T4/T5)
**Last verified:** Step 10. T3 opens the step-9 signature modal, which asks for the reason when the meaning is `Cancelled`, so there is no third modal (decision 94). A server 400 was forced with U+0085 and with U+FEFF, and it **stayed in the modal** with the reason and password kept (decision 95). A wrong password cleared only the password. Success stored the reason trimmed, and a 409 from two tabs left no `SignatureFailed` row. The same U+0085 paste into T2's email showed decision 95 holds for T2 too. Cancel refuses while dirty (decision 97, Lain's ruling). See the checkpoint.
