...
**Current step:** 7b, bind the fields. **Verified, awaiting commit.** Next: step 7c (Save Draft)
**Last verified:** Step 7b. 24 controls enabled and 20 asterisks on the owner's Initiated draft, and each half of `editable()` shown failing on its own. See the checkpoint
...
| 31 | **A partly typed or invalid date reports `value === ''`.** For example, day and month with no year, or 30 Feb. `dateOutput` turns that into `null`, so a save would **clear the stored date** with no error. Only `input.validity.badInput` tells it apart from a deliberately emptied picker | Decide at 7c |
| 32 | **Keystrokes typed while a save is in flight are lost**, because the response goes through `setRecord()` and rebuilds `form`. **Recommendation: lock the form while saving**, `&& !saving` in `editable()`. A field-by-field merge would have to tell a server trim from a user edit, which means reimplementing Go's `TrimSpace`, and the whitespace sets differ (Go counts U+0085, JS counts U+FEFF) | Decide at 7c. Lain agreed it is 7c's decision |
