...
**Current step:** 6, All + My Change Controls. **Verified, awaiting commit.** Next: step 7a
**Last verified:** Step 6. **All thirteen checks were run and passed**, including the optional one — which closed flag 12 across all three of its branches. See the checkpoint
...
| 14 | **These links 404 until their steps land.** ~~`/change-controls` and its `?state=` stat-card links~~ and ~~`/my-change-controls`~~ **landed at step 6.** Still open: `/approvals` (step 11), the Admin's `/settings/users` (step 15), and the eye links to `/change-controls/{ccId}` — now on **both** the dashboard and the list (step 7a) | Accepted, narrowed at step 6. Each stops 404ing when its step lands |
