...
**Current step:** 12, file upload. **Verified, awaiting commit.** Next: step 13 (T6 + the final decision)

**Last verified:** Step 12.
- **The upload:** the owner uploads one PDF to `implementation_evidence` through the prototype's box. Choosing the file is the upload.
- **The response** is now the whole record (Lain's backend change, defect 23), so it goes through `setRecord()` with no refetch. Because of that, the upload refuses while the form is dirty and locks it while in flight (decision 105).
- **What was proven:** replace-not-insert (the same row id), the stored name differing from the chosen one, and the 401 retry re-sending `FormData`.
- **Still open:** a status 0 under throttling. It came from `WriteTimeout`, not from the size limit (flag 61), and it is the first observation of flag 10's mechanism.
- **A6 audited**, which **completes Part A**.
