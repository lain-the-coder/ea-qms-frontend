...
**Current step:** 15, Admin user management. **Verified, awaiting commit.** Next: step 16 (activity-gated refresh), **the last step**. Step 17 is deferred to Phase 2 (decision 116), so the plan is eighteen steps and ends at 16.

**Last verified:** Step 15. **The last screen is built.**
- **`blocked_cc_ids`, declared at step 2, is finally consumed.** It is a banner under the user's row for both PUTs, with the heading chosen by the control used, not by the message (decision 118). Checks 5 and 6 saw thirty and ten CC-IDs, and psql proved nothing was written.
- **Deactivation bites on the next request** (A1.6, observed for the first time at check 7).
- ⚠️ **Three premises of Lain's brief were stale, and all came from the BACKEND's PROGRESS** (defects 14–16 and F3), which lags this repo. **A note in one repo's PROGRESS is not evidence about the other's state.** See the corrections section.
- ⚠️ **The pattern note gained a third item, and it caught its own author:** Lain's 30-minute window, and then Lain's correction of it. See the corrections section and A1.2.
- **One permanent user was spent**: `S15 Test User` (checkpoint).
...
| 14 · File download | ✅ | Lain: browser, Network tab, console and psql, across four roles. **Checks 1, 2, 5, 6, 7 and 9 pass.** **Unreachable, not skipped:** 4 (a `blob:` URL has no resource entry to fetch back) and 8 (DevTools throttles delivery to the page, not the socket). **Skipped:** 3 (keyboard). **Dropped:** 10 (flag 59 is deferred). `bun run check` (186 files, 0 errors, 0 warnings) and `bun run build` pass |
| 15 · Admin user management | ✅ | Lain: browser, Network tab and psql, across four roles. **Every check run passed**: eight numbered in Lain's report, plus the own-row, inactive-user, self-rename and Profile-hint checks. **Not reported:** the trailing-space no-op, reactivation, and the optional status 0 (plan checks 7b, 11b, 14). `bun run check` (188 files, 0 errors, 0 warnings) and `bun run build` pass |
| 16 · Activity-gated refresh | ⬜ | |
| ~~17 · Inactivity popup~~ | ⏸ | **Deferred to Phase 2** at step 15, by Lain's decision (decision 116). Not dropped |
...
**Notes for the next session:**
- ⚠️ **Every sign-out must clear `auth.user` before it navigates.** Step 17's inactivity popup was to be next to add one. It is deferred to Phase 2 (decision 116), and the rule still binds it.
...
**Built:**
- **`api.ts`:**
  - `toResult` gains `read`, a success-body reader that defaults to `json()`. An error body never reaches it, so **the status is branched on before the body is read, once, for every caller**.
  - The old body of `request()` moved, unchanged, into a private `authorised(method, path, body, read)`.
  - `request<T>` keeps its signature and calls `authorised` with the JSON reader. **No call site changed.**
  - `download(path)` calls it with `blob()`. Its `try/catch` turns a cut-off body into status 0, "The download was interrupted. Try again." (decision 111).
- **`[ccId]/+page.svelte`:**
  - **`downloadEvidence()`:** the name is taken from the record at click time (decision 112). Then a detached anchor with `download` set is clicked, and the URL is revoked immediately (decision 113).
  - **The `downloading` lock** is a plain `let` outside `editable()` (decision 115).
  - **A failure goes to the bar through `fail()`**, verbatim, unless the user has moved to another record.
  - **The name itself is the control:** a `role="button"` `.login-link` span with Enter and Space, outside `mayEdit` (decision 114).

**Documents, in the same commit:**
- **Blueprint:**
  - A6.2 is rewritten.
  - B7 gains "built at step 14".
  - B9 row 14's "Proves" is amended: it said `Content-Disposition`, which the client deliberately does not read.
- **`openapi.yaml`:** defect 27, plus the download description (branch before reading; the name from the record; read-only).
- **`api.md`:** an evidence-download section, and the blob bullet updated.
- **`svelte.md`:** the clickable-name rule.

**Rule 7: A6.2 against `HandlerDownloadFile`, `GetChangeControlIDByCcID` and `GetFileAttachment`.** The findings are in the corrections table. **Found correct**, so these are not re-investigated:
- **Bearer required.** No header gives 401 `Unauthorized`, which is `request()`'s retry string.
- **No role, owner or state check** (`main.go:159`). POST on the same path is owner-only, so the method is the only difference.
- **`Content-Disposition` and `Content-Length` are exposed** on every actual response. The copy on the preflight has no effect and does no harm.
- **`Content-Length` = `file_size` = `len(data)` at upload.** `Content-Type` is the constant `application/pdf`.
- **Two `SELECT`s and no transaction**, so there is no audit row and no `last_updated_on` move.
- **`implementation_evidence.file_name` is the same column** the header carries.

**Verified (Lain):**
- ⚠️ **Check 1, approver on CC-030:**
  - **Network:** one GET, 200, `application/pdf`, `Content-Length` 42216, matching the 41.2 KB shown.
  - **The file:** saved as `small.pdf`, and it opened correctly. The location picker is the browser's own setting.
  - **`access-control-expose-headers` does include `Content-Disposition`**, so the header IS readable. **Not reading it rests on the byte-per-character reason alone** (decision 112).
  - **Styling:** the name reads as the same size as its metadata, blue against plain text. The predicted difference is real, 13px against 14px, and not visible.
- ⚠️ **Check 2:**
  - Viewer, Admin and Owner all download on CC-026 (Closed).
  - Owner on CC-025: **the name downloads and the upload box still works.** Two controls on one field, under different gates.
- ⚠️ **Check 5:** GET 401 → `/refresh` 200 → GET 200, and the file was saved. **The shared `authorised()` gave the download the retry for free.**
- ⚠️ **Check 6:** typed in Implementation Summary, then clicked the name. **No prompt and no refusal.** One GET, the file saved, "Unsaved changes" still in the bar, and the typing intact.
- **Check 7:** spam-clicked. One GET, one file.
- **Check 9, psql:** CC-030's `last_updated_on` still reads 11:59:59 from T6, and the audit count is unchanged. **Nothing written.**
- **Me:** `bun run check` gives 186 files, 0 errors, 0 warnings. `bun run build` succeeds. `openapi.yaml` parses with 48 schemas.

**Unreachable, not skipped:**
- **Check 4, the revoke.** `performance.getEntriesByType('resource')` returns `[]` for `blob:` URLs, so there is no URL to fetch back. **Indirect evidence:** every file saved complete and opened as a PDF, so the immediate revoke did not cut a transfer.
- ⚠️ **Check 8, truncation. Unreachable locally. DO NOT RETRY IT WITH A BIGGER FILE.** At 10 kb/s on a 7 MB file:
  - "Waiting for server response" read 5.61 s, then "Request isn't finished yet".
  - The 200 and `Content-Length: 7204656` had already arrived.
  - The handler logged `file downloaded` at about 5.6 s.

  **For 7 MB to leave Go in 5.6 s, something read it at full speed.** A `Write` returns only once the kernel accepts the bytes, and TCP flow control stops a sender outrunning its receiver. **DevTools throttles delivery to the PAGE, not the socket:** Chrome's network layer reads at full speed, so the server always sees a fast reader. The upload at step 12 did time out because there the throttle sits before the socket. **The difference is where DevTools throttles, not what `WriteTimeout` covers** (flag 61).
  - **`download()`'s catch still has a purpose:** a genuinely slow link, or a dropped connection. **Nothing reachable on this setup exercises it.**

**Not verified:**
- **Check 3 (keyboard) was skipped.** It is the upload box's handler shape, and step 12's check 11 was skipped too, so **the Enter/Space pattern has never been observed.**
- **Check 10 was dropped** (Lain): flag 59 is deferred, and an RLO-named file should not sit in the data. **So flag 59's on-disk half stays reasoned.**

**Notes for the next session:**
- **Records are unchanged.** In Implementation: CC-002, 011, 012, 025. Pending Final Approval: CC-001, CC-030. Closed: CC-005, 007, 026.
- **Flags 4, 58 and 59 all wait for the next backend touch.** Flag 4's spec sync now carries defect 27 too.

### ✅ Step 15 — Admin user management (and step 17 deferred)

**Plan change first: step 17 is deferred to Phase 2**, by Lain's decision (116, recorded as a reversal of its first reason). B9 is now eighteen steps, ending at 16.

**Built:**
- **`settings/users/+page.svelte`, new.** It borrows none of the CC form's machinery, only its reasoning (decision 125).
  - **Create New User:** the four blank checks are mirrored in Go's order with Go's sentences, and **the password policy is not** (decision 121). `creating` is a synchronous guard with the label `Adding…`. On a 201 the fields are cleared, "Added {name} ({email})." shows, and the page is refetched. `autocomplete` is off on the email and `new-password` on the password, so the browser does not fill in the Admin's own sign-in.
  - **All Users:** `limit` and `offset` live in the URL, `active` is never sent (decision 122), and the pager is the second copy after `ChangeControlList`.
  - **The edit row:** one at a time. It sends only what changed, and an untouched tick closes it (decision 120). Your own row gets a name-only pencil (decision 119).
  - **The toggle** uses `preventDefault()`, so it moves only on a 200 (decision 124).
  - **Row errors:** a `.role-block-row` banner under the row, with text from one `banner()` that routes by shape (decision 118).
  - **Every response is merged by picking fields**, so `created_on` survives. A self-rename also updates `auth.user.full_name`.
- **`(app)/+layout.svelte`:** an Admin's Settings link opens `/settings/users` (decision 123, flag 18).
- **`settings/+page.svelte`:** both Profile hints now show for every role (decision 117).

**Two prototype disagreements about Profile, both settled by asking what is TRUE rather than what is drawn:**
- **The pencil (decision 119).** The Admin Profile says "Name is changed by an Admin in User Management", but `settings-admin.html` gives the `(you)` row no pencil. Followed literally, no Admin could ever correct their own name, and the hint would be untrue for them. **The pencil was added, and it makes the hint true.**
- **The hints (decision 117).** The two end-user prototypes draw read-only name and email fields with no explanation, and only the Admin prototype has the hints. Both sentences are true for every role. **The hints were added, and they make the screen tell the truth.**

**Documents, in the same commit:**
- **`openapi.yaml`:** defects 28–32.
- **`types.ts`:** the two real 409 messages on `BlockedRoleChangeResponse`.
- **Blueprint:**
  - A1.2 gains the timer's two jobs and a falsifier.
  - A1.5 and B12 record step 17 as deferred, and B9 has row 17 struck and the count at eighteen.
  - A8.2: the users 409s are not "reload". A8.3 carries the real messages.
  - A9.2: the `active` 400, `ParseBool`, and the missing tie-breaker.
  - A10: the Sidebar exception, and in User management the own-row pencil, both 409s, the no-op, the merge and **the four audit action types**.
- **`api.md`:** a User management section, and the timer's two jobs.
- **`svelte.md`:** line 68 no longer names step 17 as coming.

**Rule 7: A9's users half, A10's Settings, and the universal 409 claim**, against `handlers_users.go`, `users.sql`, `helpers.go` and `main.go`. The findings are in the corrections table. **Found correct**, so these are not re-investigated:
- A9.0: `full_name` ASC, paginated, and `total` uses the same filter.
- A9.1: the default of 50, the clamp at 200, both 400 sentences, and pagination parsed before `active`.
- A9.2: `?active=`, not `?is_active=`.
- A10: separate calls; both self guards are 400 with the documented sentences; the only 403 is `requireRole`; the self-role check fires only on a different role; Profile has no self-service route.
- ⚠️ **All-or-nothing, confirmed:** the CC guard returns before `UpdateUserName`, under `defer tx.Rollback()`. **Check 5 proved it in psql.**

**Verified (Lain):**
- **The hints:** Owner, Approver and Viewer all see both hints under the two disabled fields. The Admin's Profile is unchanged.
- **1 · Routing:** an Admin lands on User Management with one GET `?limit=20`. An Owner lands on Profile. An Owner typing `/settings/users` gets no create form, and `{"error":"Forbidden"}` verbatim, with no redirect.
- **2 · Paging at `?limit=2`:**
  - "Showing 1–2 of 7" over four pages, and the size select shows `2, 10, 20, 50` with 2 selected, so **the clamp is visible**.
  - Page 3 sent `offset=4&limit=2`, and Back returned to page 1.
  - **`?active=false` was dropped**: only `limit` was sent.
- **3 · Create refusals**, with the count unchanged at 7:
  - a blank name gets its sentence, with no request
  - `abc` gets **all four rules in one sentence** (decision 121's premise, observed)
  - `ADMIN@eaqms.local` gets a 409, so the `LOWER(email)` index works
- **4 · Create success:** the 201, the fields cleared, the notice, and a second request refetching. **The user sorted onto page 4, at offset 6, so the refetch mattered.** psql shows the Viewer row and a `UserAdded` row.
- ⚠️ **5 · Blocked role change on Default CC Owner:**
  - **thirty CC-IDs**, all bold, commas between, "and" before the last
  - the edit row stayed open with the typed name
  - **psql: name and role unchanged, and the audit count unmoved**
  - × cleared the row and the banner, with no request
- ⚠️ **6 · Blocked deactivation on Default Approver:**
  - "Deactivation blocked.", ten CC-IDs, and **no** "No part…" sentence
  - the toggle never moved, and psql still reads `is_active = true`
  - **The heading came from the action**, although the two server messages differ.
- ⚠️ **7 · Deactivation is immediate.** The test user was signed in in a private window and deactivated from the Admin window: a 200, the toggle moved, the row dimmed, and `UserDeactivated` true → false. **Their next click went straight to `/login` with the deactivated message.** A1.6, observed.
- ⚠️ **8 · Flag 29, and a third name:** Default Approver was renamed, then a Closed record they signed was opened.
  - Assigned Approver showed the **new** name.
  - **Implementation Approval By** showed the **new** name, from the `impl_by` join, a case nobody predicted.
  - Signature History showed the **old** name.

  **Two live joins and one snapshot on one screen.** The name was restored.
- **Your own row:** a name input, the role still a badge, and a disabled toggle that does nothing. A tick with no change sends nothing and closes the row.
- **An allowed edit on S15 Test User**, Viewer → CC Owner: a 200, and the badge changed. **The pencil works on an INACTIVE user, which is right:** the handler never gates on `is_active`, and an Admin may need to fix a deactivated user's details.
- **Self-rename:** a 200, and Profile showed the new name **with no request**. The name was restored.
- **Me:** `bun run check` gives 188 files, 0 errors, 0 warnings. `bun run build` succeeds. `openapi.yaml` parses. **The banner's template narrowing was checked by `bun run check` on the file as it is on disk.** At Lain's direction, no negative test was written to the working file.

**Not reported:** the trailing-space no-op (plan check 7's second half), reactivation (11), and the optional status 0 (14).

**Notes for the next session:**
- ⚠️ **The seed is SEVEN users, not four.** Agent Smoke Test, Jane B Smith (inactive) and Round3 Fixture are left over from backend testing. **Step 15 made it eight.**
- **Test data, permanent (there is no delete):** `S15 Test User`, `s15.test@eaqms.local`, password `DevPassw0rd!`. **Last reported state: CC Owner, Inactive** (deactivated at check 7, then edited). Confirm with `SELECT role, is_active FROM users WHERE email = 's15.test@eaqms.local';`. Default Approver's and the Admin's names were restored.
- **Step 16 starts from A1.2's two-jobs paragraph and its falsifier**, not from "prevents surprise logouts" or "only a hiccup".
- **Flag 4's "next backend touch" list** is in its status cell.
...
| 111 | **The download shares `request()`'s token and retry through one private `authorised(method, path, body, read)`.** `toResult` takes the success reader, defaulting to `json()`. `request` and `download` differ only in `read`. The truncation `catch` lives in `download()` alone | `request()` cannot carry a download because it parses every success as JSON, but the download needs the bearer AND the 401 retry. **The status is branched on inside `toResult`, before `read` runs**, and every error, 401s included, is JSON, so the retry logic works unchanged. Check 5 proved it. The catch is not in `toResult`, where it would change what a malformed JSON success does on every other call. **Rejected:** a second exported function copying the retry, because two copies of the refresh dance drift apart · `request()` returning the raw `Response`, which would touch every call site and hand `Response` to pages |
| 112 | **The saved name comes from `cc.implementation_evidence.file_name`, set as `a.download`, never from `Content-Disposition`** | The header IS exposed (check 1 saw it). But Go writes the name as raw UTF-8 with no `filename*`, and `Headers.get` returns one character per byte, so any non-ASCII name arrives garbled, and reading it would need a quoted-string parser besides. The record holds the same column. Because the download is of a `blob:` URL, the browser never sees the API's header anyway. **The cost is flag 63.** **Rejected:** reading the header and re-decoding its bytes, which is cleverness plus a parser, to reach a name the client already holds |
| 113 | **`URL.revokeObjectURL` straight after `click()`**, as A6.2 already said | `click()` follows the link synchronously, and resolving a `blob:` URL takes hold of the Blob itself, so the pending download survives the revoke. Left unrevoked, the Blob, up to 10 MB, lives until the page unloads. **Reasoned from the spec. Check 4 was unreachable**, and every saved file was complete. **Rejected:** a `setTimeout` revoke, a workaround for older browsers kept in reserve in case the download ever fails |
| 114 | **The file name is the control: `<span role="button" tabindex="0" class="login-link">` with Enter and Space, inside the existing `{#if cc.implementation_evidence}` and outside `mayEdit`.** Lain's ruling | No prototype draws a download control; they show the name as plain `.meta-value` text. It is never an `<a href>` (trap 4). `.login-link` is the only link colour in global.css with no layout rules: no margin, display or positioning, and no parent scoping (checked at Lain's request). A native `<button>` brings its own chrome. This is the upload box's `role="button"` pattern from the same file. **The text cursor is flag 64.** **Rejected:** the same span with `style:cursor="pointer"`, a second `style:` use (Lain: "I would rather have a wart than a pattern") · a separate `.btn secondary` Download button, a control no prototype has |
| 115 | **`downloading` is a plain `let` lock that silently ignores a second click, and it is not one of `editable()`'s locks. The download does not refuse while dirty** | A download never reaches `setRecord()`, so it rebuilds no form object, and neither the dirty refusal nor `editable()`'s locks have anything to protect. Check 6 proved the typing survives. Nothing on screen reads the lock, so it needs no `$state`. A failure is dropped if the user has moved to another record, and a success is delivered either way, as a browser download carries on after navigation. **Rejected:** a visible "Downloading…" state, because the brief was "nothing fancy" |
| 116 | ⚠️ **Step 17 (the inactivity popup) is DEFERRED to Phase 2, not dropped. Lain's decision, recorded as a REVERSAL of its first reason.** B9 is now eighteen steps, ending at 16 | **The first reason, Lain's, was WRONG:** "the refresh token has a 30-minute sliding window, so the popup warns, waits and logs you out anyway". 30 minutes is `accessTokenTTL`. `refreshInactivityWindow` is **2 hours** (`handlers_auth.go:19-21`). A1.5's popup fires at about 30 minutes idle, and **Yes** refreshes, so it can keep a session alive, and it would sign an absent user out about 90 minutes earlier than the server does. **The corrected reason, on which the deferral stands:** with step 16's gate, an idle session already ends 2 hours after its last refresh, so there is no hole. The popup changes only the *timing* of an outcome that happens either way, and 2 hours is a deliberate backend choice: if it is judged too long, the fix is `refreshInactivityWindow`, not a popup. **The case for building it later:** a forced sign-out takes unsaved work, and the guard stands aside because `signOut()` clears `auth.user` first (7d+ check 6, decision 77). The third item in the corrections pattern note. **Rejected:** building it now |
| 117 | **Both Profile field hints show for EVERY role**: "Name is changed by an Admin in User Management." and "Email cannot be changed." **Lain's catch** | ⚠️ **The prototypes are the SOURCE of the old difference, not a reason for it.** `admin/settings-profile.html` draws both hints, and the two end-user prototypes draw neither. Step 4 ported that faithfully, and nobody weighed whether it was intended. **A faithful port of an unexamined difference is still an unexamined difference.** Both sentences are true for every role: no endpoint changes an email, and `PUT /users/{id}` is `requireRole(roleAdmin)`. An Owner, Approver or Viewer can change neither field anywhere, so without the hint the greyed field looks broken. The Admin, who has a pencil since this step, needs it least. The pair of 119 (checkpoint). **Rejected:** keeping the end-user prototypes' omission |
| 118 | **A row's error is the prototype's `.role-block-row` banner directly beneath THAT row, for BOTH PUTs. It is routed by SHAPE, and its heading comes from the control used** | Decision 95's rule: an error goes where the user can act on it, and the toggle is in the row as much as the pencil is. `rowError = { id, action: 'edit' \| 'status', body }`. One `banner()` returns text segments: `blocked_cc_ids` gets the prototype's sentence, with the ids `<strong>` and joined "A, B and C", and anything else is `error` verbatim. **The heading comes from `action`**, which the client knows because it chose the endpoint (decision 90: never by message). "No part of this change has been saved" is said for the edit only, where it is true (backend decision 22). The toggle has one field and visibly never moved. Cleared on the row's next action, on Cancel and on every load. **F3 needed no action:** the prototype already reads "No part…". Checks 5 and 6. **Rejected:** the action bar or a dialog (neither exists on this page) · a heading chosen by the message · links on the CC-IDs (invention) |
| 119 | **Your own row gets a NAME-ONLY pencil. The role stays a badge and the toggle stays `disabled`.** Overrides `settings-admin.html` | The prototypes contradict each other. Admin Profile says "Name is changed by an Admin in User Management", and the `(you)` row has no pencil, so literally followed, no Admin could ever correct their own name, and the hint would be untrue. **The API allows it** (backend decision 21), refusing only a self role change and self-deactivation. **Profile stays read-only** (A10, backend F7): there is no endpoint for a non-Admin, and an Admin-only edit there would duplicate the pencil. A 200 updates `auth.user.full_name`, so Profile shows the new name with no request. The pair of 117. **Rejected:** the prototype's `—` · a name edit on Profile |
| 120 | **The edit sends only what differs from the row. An untouched tick sends NOTHING and closes the row, with no message** | Decision 65's precedent, and a stale tab cannot revert another Admin's change. ⚠️ **"No changes to save" is the WRONG precedent here:** Save Draft keeps the form open, so a silent click looks ignored. The tick ENDS the edit, so the row returning to display is the answer. The server's no-op (200, committed, **no audit row**) stays reachable through a trimmed-equal name or a stale tab, and the merge shows the truth. A blank name is refused with Go's sentence. **Rejected:** a "No changes" hint · sending both fields every time |
| 121 | **Create mirrors the four BLANK checks, in Go's order with Go's sentences, and does NOT mirror the password policy** | Decision 96: apply what the server applies, and send as typed. Name, email and role are trimmed for the check, and the password is compared to `''` only (BR-8.2.16). The role check is also what makes the body a `CreateUserRequest` without a cast. **The policy is not mirrored**, because mirroring paid at decisions 101 and 106 when the certain 400 cost something (a password in the modal, a whole upload). Here it comes before the transaction, writes nothing, loses nothing typed, and **already lists every unmet rule in one sentence** (check 3 saw all four). A mirror would copy Go's Unicode category tables into `\p{…}`, which decision 96 declined for whitespace. **Rejected:** mirroring the policy · a policy hint (not drawn) · no client checks (the body would need a cast) |
| 122 | **`limit` and `offset` live in the URL. `active` is never sent or read.** `onMount` + `afterNavigate`, deduplicated, as in `ChangeControlList` | **Verifiability:** the smallest page size is 10 and there is no delete, so a `$state` pager could only be checked by creating 11+ permanent users, where `?limit=2` shows four pages with seven (check 2). Reload and Back keep the page. The prototype has no status filter and shows both, so a hand-typed `?active=false` is dropped (decision 37's rule). The clamp stays visible (decision 39). An offset past the end reads "No users on this page", not "Showing 21–20". The pager is the second copy. **Rejected:** a `$state` pager, "less machinery" that cannot be checked without spending users |
| 123 | **An Admin's sidebar Settings opens `/settings/users`; every other role's opens `/settings`.** Closes flag 18 | As the Admin prototype's sidebar does. `href` reads `auth.user?.role`, so it reads `/settings` during the restore. The active test is unchanged. A10's "nothing in it is role-conditional" is amended to name this one exception. **Rejected:** leaving the Admin on Profile, one click from the screen only they have · redirecting `/settings` for an Admin, which would cost Profile its URL |
| 124 | **The toggle calls `preventDefault()` and moves only on a 200. There is no confirm dialog. Responses are MERGED, never replaced** | The checkbox never shows a status the server has not confirmed, and on a failure there is nothing to put back. A per-row `toggling` set, plain and not `$state`, since nothing renders it (decision 115). No confirm: no prototype or BRD draws one, and it is reversible. **The merge picks `full_name`, `role` and `is_active`**, because `UserStatusResponse` has no `created_on`. `(you)`'s toggle is `disabled`, as drawn (flag 67). **Rejected:** binding `checked` and reverting on a failure · a confirm |
| 125 | **What carried over from the CC form, and what did not.** Lain asked for it said explicitly | **Carried:** routing by shape (90) · an error where the user can act on it (95) · verbatim sentences, and checks that mirror the server's while sending as typed (96) · synchronous guards (59) · sending what changed (65) · status 0 proves nothing (trap 6). **Did not carry:** the requirements dialog (no user handler emits `issues`) · `screenKey()`/`shownError`, which exist because the bar sits far from the field, where here the banner sits under the row · pinned 409 → reload, since a users 409 is a guard, not "the record moved", and a reload would lose the edit the Admin retries with the name alone · `setRecord`'s rebuild (a field merge replaces it) · **dirty tracking and a navigation guard** (flag 66: Lain's ruling). **Nothing was imported from `[ccId]/+page.svelte`** |
...
| 14 | **These links 404 until their steps land.** ~~`/change-controls` and its `?state=` stat-card links~~ and ~~`/my-change-controls`~~ **landed at step 6.** ~~The eye links to `/change-controls/{ccId}`~~, on both the dashboard and the list, **landed at step 7a.** ~~`/approvals`~~ **landed at step 11.** ~~The Admin's `/settings/users`~~ **landed at step 15** | **Closed at step 15.** Every link resolves |
...
| 18 | ~~**The Admin's Settings link opens Profile, not User Management.**~~ The Admin prototype's sidebar goes to `settings-admin.html`. The build sent every role to `/settings` | **Closed at step 15** (decision 123). An Admin's link opens `/settings/users`. Check 1 |
...
| 29 | ~~**Live-join vs snapshot names is reasoned, not observed**~~ (check 12 was skipped). Renaming Default Approver should change every name on the form and none in Signature History | **Closed at step 15, observed** (check 8). On one Closed record, Assigned Approver **and Implementation Approval By** (the `impl_by` join, which nobody had predicted) showed the new name, and Signature History the old one. The name was restored |
...
| 59 | ⚠️ **For step 14: the saved filename can lie about itself.** `sanitizeFilename` drops only C0 controls, DEL, `"`, `'`, `` ` `` and `;`. It **keeps bidi overrides such as U+202E**. The download writes `Content-Disposition: attachment; filename="…"` with the raw name and **no `filename*`**, so nothing constrains how the name is encoded or presented. **The mechanism:** a file uploaded as `report_<U+202E>txt.pdf` passes the extension check, which reads the raw name, and is saved under that name. The user's file manager **displays it as `report_fdp.txt`**: the characters after the override are shown reversed, so the visible name, and apparently its extension, differ from the real one. **Nothing executes:** the bytes were verified as PDF on upload. The finding is that **the name misrepresents the file**. C1 controls and other non-ASCII characters pass through the same way | ~~**Step 14 owns it.** It decides whether to strip or neutralise the name on download, show a warning, or take the fix to the backend's sanitiser. On screen the name is safe, because Svelte interpolates text~~<br><br>**DEFERRED at step 14, by Lain's decision. Not scheduled, and not outstanding work.** It is an extremely rare edge case. Nothing executes, because the magic-byte check guarantees the contents are a PDF. The worst outcome is a misleading label.<br><br>⚠️ **CORRECTED CLAIM. The original was WRONG, not incomplete.** "On screen the name is safe, because Svelte interpolates text" conflated markup injection with bidi reordering. Interpolation stops the first and does nothing about the second: `report_<U+202E>txt.pdf` renders as `report_fdp.txt` in the `.meta-value`, and since step 14 that text is the download control. **Do not trust the reasoning that produced the struck sentence.**<br><br>**What step 14 changed:**<br>- **The `Content-Disposition` half is moot for this client.** The name comes from `a.download` (decision 112), and the browser never sees the API's header for a `blob:` URL. It still applies to curl `-OJ`, Postman and `/docs`.<br>- **The on-disk half is REASONED, NOT VERIFIED.** The browser's own download sanitiser is believed to replace these characters (Chromium's treats format characters as illegal). Check 10, which would observe it, was dropped with the flag's deferral.<br><br>**No client workaround, for flag 24's reason:** a frontend strip would show a different name from Postman and curl. **The fix, whenever the backend is next touched** (alongside flag 4's spec sync and flag 58): `sanitizeFilename` drops **U+202A–202E, U+2066–2069, U+200E, U+200F, U+061C** |
...
| 63 | **A download can save new bytes under an old name.** The name comes from the record, taken at click time (decision 112). If another tab replaced the file under a different name after this page loaded, the GET returns the new file and `a.download` gives it the old name | **Accepted.** Both names are sanitised `.pdf`s, and the bytes are always current. Replacement happens only in `In Implementation`, by the owner. Any load or upload response corrects it, and a reload is the workaround. Reasoned, not observed |
| 64 | **The download name shows a text cursor on hover, not a pointer.** `.login-link` sets no `cursor`, and no class in global.css gives a span one | **Accepted by Lain as the cost of not adding a second `style:` override** (decision 114): "I would rather have a wart than a pattern". ⚠️ **Recorded so a future step that legitimately needs a second inline style weighs both together.** If `style:` gains a second use, `style:cursor="pointer"` on this span should be decided at the same time |
| 65 | ⚠️ **BACKEND — `GET /users` sorts with NO tie-breaker** (`users.sql:22`, `ORDER BY full_name` alone). Postgres does not order ties stably, so two users with the same full name can swap between requests. **Across a page boundary, one may appear on both pages or on neither.** Found by step 15's A9 audit | **Not worked around client-side**, for flag 24's reason. The fix is `ORDER BY full_name, id`. **On flag 4's "next backend touch" list.** Recorded in A9.2 and the spec (defect 32). Unobserved, because no two seeded names are equal |
| 66 | **The user page has no navigation guard and no dirty tracking.** An open edit is dropped with no prompt by another pencil, a page change, a create's refetch, or leaving. A half-typed create form is dropped by leaving | **Accepted, Lain's ruling.** The CC form's guard was almost free, because `dirty` already existed for the save diff. Here, change-tracking would be built **solely to power a prompt**. What is lost is a name or four create fields, not a 2000-character comment. The prototype has none, B4 says invent nothing, and **the cost is a retype** |
| 67 | **The `(you)` toggle is `disabled` but looks enabled.** `.toggle-switch` sets `cursor: pointer` and has no disabled style, so a click does nothing and says nothing (decision 74's objection to a disabled `.btn`) | **Accepted.** The prototype, A10 and backend F4 all draw it disabled, and the row is marked `(you)`. The server would refuse with `Self Deactivation is not allowed`. Observed at step 15: the click does nothing |
| 68 | **A user whose role changes keeps the old role's UI until they reload.** `auth.user.role` comes from the sign-in or `/me`, and nothing refetches it | **Accepted.** The server enforces the new role on every request, because `middlewareAuth` re-reads the user, so the stale UI can only draw a 403. Deactivation is different: it ends the session at once (A1.6, observed at step 15 check 7). Unobserved |
| 69 | ⚠️ **BACKEND — `POST /users` never validates the email's FORMAT.** It is trimmed and checked for blank only, so `abc` is a valid email. It is stored as typed, with uniqueness case-insensitive. The spec claimed `format: email` (defect 31) | **Not worked around client-side**, for flag 24's reason: the UI would refuse what Postman and curl accept. **On flag 4's "next backend touch" list.** The prototype's `type="email"` validates nothing outside a `<form>` submit |
| 70 | **The user table's `{#each}` is UNKEYED (B3), so DOM nodes follow POSITION, not the user.** If rows reordered under an open edit, the name input would be destroyed at its old position and rebuilt at the new one. ⚠️ **The typed name is SAFE**, because it lives in `editing`, stored under the user's id, not in the DOM. So it follows the user. **What is lost is DOM-only state:** focus, the caret, and any half-finished IME input. A keyed `(u.id)` would move the same element and keep them. Lain's framing ("the input could carry its typed value onto a different row") was checked against the mechanism and narrowed to this | **Unreachable today:** `load()` sets `editing = null` before its `await` (a page change, a create's refetch), `merge()` edits fields in place and never reorders, and nothing polls. ⚠️ **What would make it reachable:** re-sorting rows on the client after a rename, or any load that keeps `editing` open (a background refresh, or a create that no longer clears the edit). **Whoever adds either must key the `{#each}` or clear the edit.** A comment at the `{#each}` points here |
...
> ⚠️ **A second pattern (step 14): a wrong claim written down AS A CORRECTION.**
> It happened twice in one day, so it is a pattern, not an incident.
> - **Flag 59** recorded "on screen the name is safe, because Svelte interpolates
>   text". It was plausible, it was written as the flag's own safety claim, and it
>   conflated markup injection with bidi reordering.
> - **Flag 61:** check 8's observation (a throttled 7 MB download that did not time
>   out) was correct, and Lain proposed narrowing the flag to uploads on the
>   mechanism "`w.Write` hands the bytes to the kernel and returns". Claude refused
>   to write it, because a correction resting on a false mechanism is the flag-59
>   failure repeating. The mechanism that explains the observation is where
>   DevTools throttles, not what `WriteTimeout` covers.
>
> **A correction is trusted more than the claim it replaces, so it needs the same
> check against the Go, not less.** When an observation contradicts a flag, first
> ask whether the observation could have been produced by the test setup rather
> than the system. Then write down what observation would falsify the new claim.
>
> - ⚠️ **Third item (step 15), and the note caught its own author.** Lain's brief
>   deferred step 17 on "a 30-minute sliding window, so the popup logs you out
>   anyway". 30 minutes is `accessTokenTTL`, and `refreshInactivityWindow` is **2
>   hours** (`handlers_auth.go:19-21`), so the popup would have signed an absent
>   user out about 90 minutes early, and Yes keeps a session alive. Lain then
>   offered a correction for step 16: "the timer prevents a 401-refresh-retry
>   hiccup, not a sign-out". **Claude refused to write it, by applying this note
>   to the correction.** It is true for a user who sends requests. It is false for
>   one who interacts and sends none for over 2 hours, the long unsaved draft,
>   where the timer prevents a sign-out that loses work. A1.2 now states both jobs,
>   with a falsifier. **All three items are claims about MECHANISM, checked against
>   the source and refused.**
>
> ⚠️ **A third pattern (step 15): stale evidence handed over as fact.** Three
> premises of Lain's step-15 brief came from the **backend's** PROGRESS, pasted in
> as current: defects 14–16 "never fixed" (fixed here at steps 4 and 7a, not 7c),
> and F3's "false sentence" in the prototype (the prototype has read "No part of
> this change has been saved" since the first commit). The backend's notes lag
> this repo. **A note in one repo's PROGRESS is not evidence about the other
> repo's state.** Check the file the note is about.
...
| ✅ **Rule 7 for step 14: A6.2 against `HandlerDownloadFile`, `GetChangeControlIDByCcID` and `GetFileAttachment`** | The claims found correct are in the step 14 checkpoint. **Five gaps and one spec defect**, below | Rows below |
| ⤷ ⚠️ **gap · A6.2's snippet was a raw `fetch` with no retry** | It contradicted B7 ("no raw fetch") and would 401 once the 30-minute access token expired | A6.2 now goes through `download()`. Check 5 proved the retry |
| ⤷ ⚠️ **gap · A6.2 never branched on status** | `blob()` reads a JSON error body happily, so the snippet as written would save `{"error": …}` to disk as the PDF | Stated in A6.2, `api.md` and the spec. `toResult` branches before `read` runs |
| ⤷ **gap · "the try/catch also covers truncation", but the snippet had none** | And "throws if truncated" was presented as a detail, when it is the only failure reachable after a 200 | `download()` has the catch. **Reachability is REASONED** (flag 61's download note); check 8 could not produce it locally |
| ⤷ **gap · the filename source was not stated** | "`Content-Disposition` carries the filename" invites reading it. It is exposed, but Go writes raw UTF-8 and `Headers.get` returns one character per byte | A6.2 and `api.md`: take the name from the record, as `a.download` (decision 112) |
| ⤷ **gap · no error list, and nothing on side effects** | The handler's order, UI reachability (none), and "writes nothing" were all absent. Check 9 confirmed the last | Listed in A6.2, with "none reachable from the UI" |
| ⤷ **defect 27 · the download's 400 list** (`openapi.yaml`) | Only the `fieldName` case was listed. `CC-ID cannot be blank` was missing; curl can reach it with `%20`. Same class as defect 25 | A table of both, in the handler's order. The description gains the status branch, the name source and "read-only". **Waits for flag 4's route** to reach the backend's copy |
| ⤷ **B9 row 14's "Proves" named `Content-Disposition`** | The client deliberately does not read it | Amended to blob handling through the shared retry, the status branch, and the name from the record |
| ✅ **Step 15 audit: A9's users half and A10's Settings** against `handlers_users.go`, `users.sql`, `helpers.go` and `main.go` | **Found correct, not to be re-investigated:** the sort, pagination and `total` (A9.0, A9.1) · `?active=`, not `?is_active=` · the separate calls · both self guards 400 with the documented sentences · the only 403 is `requireRole` · the self-role check fires only on a different role · Profile has no self-service route · **all-or-nothing**, because the CC guard returns before `UpdateUserName` (check 5 proved it) | The gaps are below |
| ⤷ **gap · A9.2: the `active` 400 and `ParseBool`** | An invalid value is 400 `Invalid Query Parameter for Active`. `1`, `t`, `TRUE` and the like are accepted | Added to A9.2 and the spec (defect 32) |
| ⤷ ⚠️ **gap · BACKEND: no tie-breaker on `ORDER BY full_name`** | Flag 65 | In A9.2 and the spec |
| ⤷ **gap · A10 silent on the 409s, the no-op and the merge** | Both PUTs return `blocked_cc_ids`. Both have a 200 no-op that writes no audit row. `UserStatusResponse` has no `created_on` | A10's User management paragraph now covers all three |
| ⤷ ⚠️ **gap · A10 never listed the user AUDIT ACTION TYPES**, which **Lain found at check 8** | A role change writes **`UserRoleChanged`** (`field_name` `role`), not `UserUpdated`. **The full set, from the Go:** `UserAdded` (create) · `UserUpdated` + `full_name` (rename) · **`UserRoleChanged`** + `role` · `UserDeactivated` + `is_active` true→false · `UserUpdated` + `is_active` false→true (reactivate). A name and a role in one request write two rows with one timestamp. **`UserUpdated` means two things**, so read `field_name`. **Not added later, and backend decision 20 is not incomplete:** 20 is scoped to `PUT …/active` alone, and covers it fully. `UserRoleChanged` has been in `ck_audit_logs_action_type` since the migration (`112366d`, 2026-07-19), was first written by `PUT /users/{id}` (`c46edd2`, 2026-07-30), and the backend's own PROGRESS records it at checkpoint 16 ("`UserUpdated` for the name, `UserRoleChanged` for the role"). **The gap was on the frontend side.** The step-15 plan's check 8 did name `UserRoleChanged`, but A10 had nothing | A table of all five cases in A10 |
| ⤷ ⚠️ **defect · A8.2's 409 row, a universal claim** ("refetch the record") | A sweep of every `StatusConflict` found **eleven** 409s: two saves, five transitions and the upload, which are the **eight** change-control ones, all meaning "the record left the state". The plan said "nine", and that miscount was caught before the commit. **The three user ones do not:** a duplicate email, and the two `blocked_cc_ids` guards. None is stale, and a reload would lose the edit | A8.2 splits the row, and `api.md` says "do not reload" |
| ⤷ ⚠️ **`openapi.yaml` defects 28–32 · the users section** | **28:** the `POST /users` 400 example was not a real message. The Go joins the policy as `Password must contain at least 8 characters, at least 1 uppercase letter, …`, and the other 400s were unlisted. **29:** `maxLength: 100` on both `full_name`s. The Go has no limit and the column is `TEXT`. **30:** the `BlockedRoleChangeResponse` example message was never sent. The real ones are `Cannot update role of a user with active CCs` and `Cannot deactivate a user with active CCs`. Blueprint **A8.3** had the same message. **31:** `format: email`, which the Go never checks (flag 69). **32:** `GET /users`'s 400 omitted `Invalid Query Parameter for Active`, and "sorted by full name" omitted the missing tie-breaker | All fixed in this copy, and A8.3 too. **Not in the backend's copy** (flag 4) |
| ⤷ **stale · the step-15 brief's "defects 14–16, found at step 7c and never fixed"** | 14 and 15 were found at step 4 and 16 at step 7a, and all three were **fixed in this copy** then. What was never done is flag 4's sync, and the backend copy still shows the 403 | No change here. The lesson is in the pattern note above |
| ⤷ **stale · backend PROGRESS F3** (`ea-qms-backend/PROGRESS.md`:1918) | It says the prototype banner reads "The name change can still be saved". `settings-admin.html:248-249` has read "No part of this change has been saved" since `5dbe85d`, the first commit | The backend's file is Lain's. Listed on flag 4's backend-touch list |
| ⤷ **stale · backend PROGRESS F1** (:1916), "without it users hit surprise logouts" | The 401 path's refresh also advances the window, so a user who sends requests is never signed out without the timer. The timer's two real jobs are in A1.2 | Same: on flag 4's list |
| ✅ **A1.2 and A1.5 · step 17 deferred, and the timer's two jobs** (decision 116) | A1.2 said the popup was "optional", and A1.5 said "build it last". Neither said what the timer does and does not prevent | A1.5 records the deferral and the corrected reason. A1.2 gains the two jobs and a psql falsifier for step 16. B9 has row 17 struck and the count at eighteen. B12 and `svelte.md`:68 are updated |
