...
**Current step:** 11, approver flow (T4/T5). **Verified, awaiting commit.** Next: step 12 (file upload)
**Last verified:** Step 11. `/approvals` is the list with `assigned=me`, one gate at a time and no "All" (decision 100). T4/T5 opens the step-9 modal with the meaning chosen by the decision. A **full loop on CC-026** (T2 → T5 reject → revise → T2 → T4 approve) put the leftover note on screen for the first time, in both its variants (flag 28), and audited `decision` twice. A U+0085 paste's 400 stayed in the modal (decision 102). The approver's typed decision now has a navigation guard of its own (decision 103). See the checkpoint.
...
| 48 | **T6's rule, "Actual Implementation Date cannot be in the future", is not yet in `submitGate`,** and the input has no `max`. Step 9 added only T2's business-day rules, behind `current_state === 'Initiated'` | Deferred to **step 13**, which wires T6. Add the rule as a second state arm, with the Go's sentence (`After(today)` in UTC), plus `max` as an affordance per A5.4, following decision 91's shape |
...
| **`openapi.yaml`** | Hand-written from the handler code — a transcription, so **not infallible**. If a response disagrees, check the Go handler and fix the spec. **Twenty-two defects found so far**: six at step 2, three more on a second reading of the same file, two in the A1 audit, two in step 3's sweep of every 401, two in step 4's A10 audit, one in step 7a's audit (the empty-signatures sentence), one in step 7b's A3 audit (`""` on `assigned_approver_id`), four in step 9's check of all five transitions (18–21), and one in step 10's T3 audit (22, runes after trimming) — so treat this as a live warning, not a formality. **The count keeps rising because the checking keeps going**, not because the spec is unusually bad |
