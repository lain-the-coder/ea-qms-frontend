...
**Current step:** 7d, Dirty tracking. **Verified, awaiting commit.** Next: step 7d+ (the navigation guard, flag 43), then step 8 (`Initiated` role views)
**Last verified:** Step 7d. Both halves of the partial-date tracker proven separately (keyboard into an empty picker, and the mouse-only popup), and "Saved 22:37" returning after an undo, with psql showing that the typing wrote nothing. See the checkpoint
...
| 7d+ · Navigation guard | ⬜ | Scoped by flag 43: its shape, the sign-out trap and seven checks |
...
| 36 | **No `maxlength` on the text fields.** The server's 400 catches over-length text. The prototypes carry none on these fields, and HTML's `maxlength` counts UTF-16 code units where Go counts runes, so the two would disagree on emoji | Accepted |
