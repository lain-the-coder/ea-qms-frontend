...
**Current step:** 4, authenticated layout. **Verified, awaiting commit.** Next: step 5
**Last verified:** Step 4. Ten of eleven browser checks were run and passed. The API-down check was skipped (flag 12), and `request()`'s refresh-failure branch was not re-run (flag 13). See the checkpoint
...
| 14 | **Four links 404 until their steps land:** `/change-controls` (step 6), `/approvals` (step 11), `/my-change-controls` (step 6) and the Admin's `/settings/users` (step 15) | Accepted. Each stops 404ing when its step lands |
