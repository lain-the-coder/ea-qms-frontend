...
**Current step:** 7c, Save Draft. **Verified, awaiting commit.** Next: step 7d (dirty tracking)
**Last verified:** Step 7c. A diff body proven in psql against both counterexamples (two tabs, TIME seconds), an atomic 400 that rolled back an audit row, and flag 31's partial date blocked. See the checkpoint
...
| 35 | **A save's 400 is not shown next to the field**, which A8.2 asks for. The message names the field in prose and sits in the action bar. Placing it would mean parsing the prose back to a field key | Accepted for 7c. Revisit at step 9, where T2's `issues` lists field labels, up to 22 of them |
