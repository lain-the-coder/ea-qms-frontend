...
**Current step:** 14, file download. **Verified, awaiting commit.** Next: step 15 (Admin user management)

**Last verified:** Step 14. **Every Part A section now has a built client.**
- **`download()` shares `request()`'s token and 401 retry** through one private `authorised(…, read)`. Only the success reader differs. Check 5 saw the retry work on a download with no download-specific code.
- **The evidence name is the control**, for every role in every state. The file is saved under the record's name, and the object URL is revoked straight after the click.
- ⚠️ **Two wrong claims were caught before they were written down.** The first was flag 59's "safe on screen". The second was a narrowing of flag 61, which Lain proposed and I refused. **That is a pattern now, not an incident.** See the corrections section.
- **Nothing was spent.** A download writes nothing (check 9).

See the checkpoint. **Flag 61 was NOT narrowed:** it gains a download note, which is reasoned, not observed.
