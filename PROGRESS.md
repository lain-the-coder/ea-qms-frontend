...
**Current step:** 9, T2 submit and the e-signature modal. **Verified, awaiting commit.** Next: step 10 (T3 cancel)
**Last verified:** Step 9. T2 ran end to end with a real signature. A wrong password produced one 401, no `/refresh` and exactly one `SignatureFailed` row. Success produced a POST and a GET of the signatures, **with no GET of the record**. A 409 left no `SignatureFailed` row. The first design put the requirements list in the sticky bar; check 1 showed twenty labels squeezing the buttons onto three lines, so the list moved to a dialog (decision 90). The narrow-window check then showed the squeeze was caused by `.btn`, not by the list (flag 39). See the checkpoint
...
| 49 | **T3's reason field: does it join the e-signature modal, or get its own?** T3 collects a reason and credentials together. It would be the third `.modal` block, and the second copy of the credential markup | **Decide at step 10**, under the three-copies rule. Flag 53 is linked: where T3's reason errors show |
...
| **`openapi.yaml`** | Hand-written from the handler code — a transcription, so **not infallible**. If a response disagrees, check the Go handler and fix the spec. **Twenty-one defects found so far**: six at step 2, three more on a second reading of the same file, two in the A1 audit, two in step 3's sweep of every 401, two in step 4's A10 audit, one in step 7a's audit (the empty-signatures sentence), one in step 7b's A3 audit (`""` on `assigned_approver_id`), and four in step 9's check of all five transitions (18–21) — so treat this as a live warning, not a formality. **The count keeps rising because the checking keeps going**, not because the spec is unusually bad |
